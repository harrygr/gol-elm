...
var $author$project$Main$init = function (size) {
	return _Utils_Tuple2(
		{
			grid: A3($LesleyLai$elm_grid$Grid$repeat, size, size, 0),
			size: size
		},
...
var $LesleyLai$elm_grid$Grid$nextIndex = F2(
	function (_v0, _v1) {
		var x = _v0.a;
		var y = _v0.b;
		var w = _v1.a.w;
...
var $LesleyLai$elm_grid$Grid$indexedMap = F2(
	function (func, grid) {
		var w = grid.a.w;
		var h = grid.a.h;
		...
		var _v1 = A3(
			...
			F2(
				function (e, _v2) {
					...
					var acc = _v2.c;
					var _v3 = A2(
						$LesleyLai$elm_grid$Grid$nextIndex,
						_Utils_Tuple2(x, y),
						grid);
					var x1 = _v3.a;
					var y1 = _v3.b;
...
var $author$project$Main$countNeighbors = F3(
	function (grid, x, y) {
		var w = A2(
...
var $author$project$Main$processCell = F4(
	function (grid, x, y, isAlive) {
		var c = A3($author$project$Main$countNeighbors, grid, x, y);
		if (isAlive === 1) {
			return ((c < 2) || (c > 3)) ? 0 : 1;
		} else {
			return (c === 3) ? 1 : 0;
...
var $author$project$Main$evolve = function (grid) {
	return A2(
		$LesleyLai$elm_grid$Grid$indexedMap,
		$author$project$Main$processCell(grid),
		grid);
};
		...
		var toggled = A2(
			...
			A2(
				...
				A2(
					...
					A2($LesleyLai$elm_grid$Grid$get, coords, grid))));
		return A3($LesleyLai$elm_grid$Grid$set, coords, toggled, grid);
	});
			...
			case 'Evolve':
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							grid: $author$project$Main$evolve(model.grid)
						}),
					$elm$core$Platform$Cmd$none);
			case 'Toggle':
				var coords = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							grid: A2($author$project$Main$toggle, coords, model.grid)
						}),
					$elm$core$Platform$Cmd$none);
			case 'Reset':
				return $author$project$Main$init(model.size);
			case 'SetSize':
				var size = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
	...
	function (x, y, a) {
		if (a === 1) {
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('cell alive'),
			...
			return A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('cell dead'),
						$elm$html$Html$Events$onClick(
						$author$project$Main$Toggle(
							_Utils_Tuple2(x, y)))
					]),
...
var $author$project$Main$renderGrid = function (grid) {
	return $elm$core$Array$toList(
		A2(
			$elm$core$Array$map,
			$author$project$Main$renderRow,
			$LesleyLai$elm_grid$Grid$rows(
				A2($LesleyLai$elm_grid$Grid$indexedMap, $author$project$Main$renderCell, grid))));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Main$renderGrid(model.grid))
			]));
};
var $author$project$Main$main = $elm$browser$Browser$sandbox(
	{
		init: $author$project$Main$init(50),
